// ========================================
// صفحة البلاغات المغلقة - Closed Reports Page
// ========================================
...
async function loadCurrentUser() {
  try {
    ...
    console.log('🔐 معلومات المستخدم (البلاغات المغلقة):', { 
      isClusterManager, 
      userHospitalId, 
...
function updateUserInfo() {
  ...
  if (userInfoElement && window.closedI18n) {
    const t = window.closedI18n.t;
    if (isClusterManager) {
  ...
  try {
    ...
    if (result.success && result.data) {
      ...
      if (!isClusterManager && userHospitalId) {
        ...
        console.log(`👤 موظف عادي - عرض بلاغات مغلقة لمستشفى واحد فقط (ID: ${userHospitalId})`);
      } else {
        console.log('✅ مدير تجمع - عرض جميع البلاغات المغلقة');
      }
      
      closedData = filteredReports;
      ...
      console.log('تم تحميل البلاغات المغلقة:', closedData.length, 'بلاغ');
    }
    
  } catch (error) {
    console.error('خطأ في تحميل البلاغات المغلقة:', error);
...
function updateSummaryCards(summary) {
  // إجمالي البلاغات المغلقة (بناءً على البيانات المفلترة)
  const totalElement = document.getElementById('closed-total');
  if (totalElement) totalElement.textContent = closedData.length;
  
  // عدد المستشفيات التي أغلقت بلاغات (بناءً على البيانات المفلترة)
  const hospitalsElement = document.getElementById('closed-hospitals');
  if (hospitalsElement) {
    if (!isClusterManager && userHospitalId) {
      // إذا كان موظف عادي، اعرض 1 مستشفى فقط
      hospitalsElement.textContent = closedData.length > 0 ? 1 : 0;
    } else {
      // إذا كان مدير تجمع، احسب عدد المستشفيات الفريدة
      const uniqueHospitals = new Set(closedData.map(report => report.HospitalID));
      hospitalsElement.textContent = uniqueHospitals.size;
    }
  }
  
  // أكثر نوع تكراراً (بناءً على البيانات المفلترة)
  const topTypeElement = document.getElementById('closed-top-type');
  if (topTypeElement) {
    if (closedData.length === 0) {
      topTypeElement.textContent = '–';
    } else {
      // حساب أكثر نوع تكراراً من البيانات المفلترة
      const lang = window.closedI18n?.getLanguage() || 'ar';
      const typeCounts = {};
      closedData.forEach(report => {
...
function renderAllReports() {
  ...
  if (closedData.length === 0) {
    const t = window.closedI18n?.t || ((key) => key);
    container.innerHTML = `
  ...
  container.innerHTML = closedData.map(report => {
    ...
    const typeName = lang === 'ar'
      ? (report.TypeNameAr || report.TypeName || ((window.closedI18n?.t('report-undefined')) || 'غير محدد'))
      : (report.TypeNameEn || report.TypeNameAr || report.TypeName || ((window.closedI18n?.t('report-undefined')) || 'Not specified'));

    return `
      <div class="bg-white rounded-xl p-6 shadow-lg border border-green-100 hover:shadow-xl transition-shadow">
        <div class="flex items-start justify-between mb-4">
          ...
          <span class="px-3 py-1 bg-green-100 text-green-700 rounded-full text-sm font-medium">
            ${translateStatus(report.StatusCode)}
          </span>
        </div>
        
        <div class="space-y-2 mb-4">
          <div class="flex justify-between">
            <span class="text-gray-500 text-sm">${(window.closedI18n?.t('report-type-label')) || 'النوع:'}</span>
            <span class="text-gray-800 text-sm font-medium">${typeName}</span>
          </div>
          <div class="flex justify-between">
            <span class="text-gray-500 text-sm">${(window.closedI18n?.t('report-department-label')) || 'القسم:'}</span>
            <span class="text-gray-800 text-sm font-medium">${report.DepartmentName || ((window.closedI18n?.t('report-undefined')) || 'غير محدد')}</span>
          </div>
          <div class="flex justify-between">
            <span class="text-gray-500 text-sm">${(window.closedI18n?.t('report-priority-label')) || 'الأولوية:'}</span>
            <span class="text-gray-800 text-sm font-medium">${translatePriority(report.PriorityCode)}</span>
          </div>
          <div class="flex justify-between">
            <span class="text-gray-500 text-sm">${(window.closedI18n?.t('report-created-label')) || 'تاريخ الإنشاء:'}</span>
            <span class="text-gray-800 text-sm font-medium">${formatDate(report.CreatedAt)}</span>
          </div>
          <div class="flex justify-between">
            <span class="text-gray-500 text-sm">${(window.closedI18n?.t('report-closed-label')) || 'تاريخ الإغلاق:'}</span>
            <span class="text-gray-800 text-sm font-medium">${formatDate(report.UpdatedAt || report.ClosedAt || report.CreatedAt)}</span>
          </div>
        </div>
        
        <div class="pt-4 border-t border-gray-100">
          <a href="${DETAILS_PAGE}?${params}"
             class="block text-center w-full bg-green-50 text-green-700 py-2 px-4 rounded-lg hover:bg-green-100 transition-colors text-sm font-medium">
            ${(window.closedI18n?.t('report-details-button')) || 'عرض التفاصيل'}
          </a>
        </div>
      </div>
    `;
...
function renderByHospital() {
  const container = document.getElementById('closed-hospitals-grid');
  if (!container) return;
    ...
    <div class="bg-white rounded-xl p-6 shadow-lg border border-green-100">
      <div class="flex items-center gap-3 mb-4">
        ...
        <div>
          ...
          <p class="text-sm text-gray-600">${(window.closedI18n?.t('reports-hospital-count', { count: hospital.reports.length })) || `${hospital.reports.length} بلاغ مغلق`}</p>
        </div>
      </div>
      ...
      <div class="space-y-2">
          ...
          return `
            <a href="${DETAILS_PAGE}?${params}" class="block">
              <div class="flex items-center justify-between p-3 bg-green-50 rounded-lg hover:bg-green-100 transition-colors cursor-pointer">
                ...
                <span class="text-xs text-green-600 font-medium">${translateStatus(report.StatusCode)}</span>
              </div>
            </a>
          `;
        }).join('')}
        ...
        ${hospital.reports.length > 3 ? `
          <div class="text-center pt-2">
            <span class="text-sm text-gray-500">${(window.closedI18n?.t('reports-more-count', { count: hospital.reports.length - 3 })) || `و ${hospital.reports.length - 3} بلاغات أخرى`}</span>
          </div>
        ` : ''}
      </div>
    </div>
...
function updateTopTable() {
  const tableBody = document.getElementById('closed-top-table');
  if (!tableBody) return;
